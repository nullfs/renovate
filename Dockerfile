...
RUN --mount=type=tmpfs,target=/tmp --mount=type=bind,target=/mnt <<-EOF
    ...
    PATH=$PWD/java/bin:$PATH && pushd jdk

    bash configure --with-jvm-features=link-time-opt \
        ...
        --with-source-date=2024-01-01 \
        --with-version-string=${JDK_VERSION:4} \
        --disable-absolute-paths-in-output \
        --enable-headless-only \
        --enable-linktime-gc && make images

    PATH=$(echo $PWD/build/*/images/jdk/bin):$PATH && popd
